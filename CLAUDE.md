...
A complete two-player **Chain Reaction** game shipped as a single self-contained file (`chain-reaction.html`) — HTML, CSS, and vanilla JS in one document, no build step, no dependencies, no framework. Open it directly in a browser to run it. Alongside it live two authoritative spec documents and a folder of validation scripts.

## Commands

- **Run the game:** open `chain-reaction.html` in any modern browser. There is no build, bundler, package manager, or dev server.
...
- `chain-reaction.html` — the implementation of both.

**Both spec docs carry an explicit maintenance rule: any change to the game must ship in the same change as the matching doc update, plus a changelog row.** When you modify `chain-reaction.html`, update the relevant doc(s) too. Do not let them drift.
...
- The JS resolution logic in `chain-reaction.html` mirrors the validated Python reference `validation/engine.py`. Keep them equivalent; `validation/` exists to prove that.

## Implementation architecture (`chain-reaction.html`, one IIFE)
